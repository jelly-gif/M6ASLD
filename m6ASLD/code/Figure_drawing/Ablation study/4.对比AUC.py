...
# 1
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'CGR_position/FprAndTpr.csv')
P = []
R = []
counter = 0
...
# # 2
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'WAL/FprAndTpr.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
# 3
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'WGG/FprAndTpr.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
# 4
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'WSM/FprAndTpr.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
# # 5
# PAndRAttribute = []
# ReadMyCsv(PAndRAttribute, 'WSMA/FprAndTpr.csv')
# P = []
# R = []
# counter = 0
# while counter < len(PAndRAttribute):
...
# 6
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'All/FprAndTpr.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.legend(bbox_to_anchor=(0.95, 0.43), fontsize='medium')
plt.text(-0.17, 1.1, 'A', fontsize=12,weight='bold')
plt.legend(prop={'weight': 'bold','size':15})
# plt.savefig('ROC5fold.svg')
plt.savefig('Ablation_AUC_(A).tif',dpi=350,bbox_inches='tight')
