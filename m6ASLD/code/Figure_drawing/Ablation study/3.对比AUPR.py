...
#1
PAndRAttribute = []
# ReadMyCsv(PAndRAttribute, 'PAndRKNN.csv')
ReadMyCsv(PAndRAttribute, 'CGR_position/PAndR.csv')
P = []
R = []
counter = 0
...
plt.plot(P, R, color='orange',
         label=r'WPC',linestyle='--',
         lw=2, alpha=1)

# 2
PAndRAttribute = []
# ReadMyCsv(PAndRAttribute, 'PAndRMannerBagging.csv')
ReadMyCsv(PAndRAttribute, 'WAL/PAndR.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='red',
         label=r'WJT',linestyle='-.',
         lw=2, alpha=1)


# 3
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'WGG/PAndR.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
    ...
    counter = counter + 1
roc_auc = auc(P, R)
plt.plot(P, R, color='deepskyblue',
         label=r'WMG',linestyle=':',
         lw=2, alpha=1)


# 4
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'WSM/PAndR.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='green',
         label=r'WCS',linestyle='dashed',
         lw=2, alpha=1)

# # 5
# PAndRAttribute = []
# ReadMyCsv(PAndRAttribute, 'WSMA/PAndR.csv')
# P = []
# R = []
# counter = 0
# while counter < len(PAndRAttribute):
...
#     counter = counter + 1
# roc_auc = auc(P, R)
# plt.plot(P, R, color='pink',
#          label=r'WSMA',linestyle=':',
#          lw=2, alpha=1)


# 6
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'All/PAndR.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.xlabel('Recall',fontsize=18,weight='bold')
...
plt.legend(bbox_to_anchor=(0.36, 0.43), fontsize='medium')

plt.text(-0.13, 1.1, 'B', fontsize=12,weight='bold')
plt.legend(prop={'weight': 'bold','size':15})
# plt.savefig('PR5fold.svg')
plt.savefig('Ablation_AUPR_(B).tif',dpi=350,bbox_inches='tight')
